...
OUTPUT_DIR  = r'D:\\AALUCE\\XGB\\XGB_result250_newpic5'
...
plt.scatter(y_train, y_pred_train, alpha=0.6, s=35,
            label='Train', color='#1f77b4', edgecolor='k')
plt.scatter(y_test, y_pred_test, alpha=0.7, s=40, marker='^',
            label='Validation', color='#d62728', edgecolor='k')

y_min, y_max = min(y_test.min(), y_train.min()), max(y_test.max(), y_train.max())
plt.plot([y_min, y_max], [y_min, y_max], 'k--', lw=2, label='1:1 Line (y = x)')
plt.xlabel('Actual Values', fontsize=12)
plt.ylabel('Predicted Values', fontsize=12)
plt.title('XGBoost Model Fit (Train vs Validation)', fontsize=14)
...
plt.text(0.97, 0.03, metrics_text,
         transform=plt.gca().transAxes,
         fontsize=10, va='bottom', ha='right',
         bbox=dict(boxstyle='round,pad=0.5', fc='wheat', alpha=0.5))

fit_plot_path = os.path.join(OUTPUT_DIR, 'xgb_fit_train_vs_validation.pdf')
plt.savefig(fit_plot_path, bbox_inches='tight', dpi=200)
print(f"[OK] Saved: {fit_plot_path}")
plt.close()
# === 代码块结束 ===
